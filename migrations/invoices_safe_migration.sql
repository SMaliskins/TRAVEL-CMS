-- =================================================
-- SAFE INVOICE MIGRATION
-- Handles case when tables don't exist
-- =================================================

-- STEP 1: Cleanup (safe - ignores if not exists)
DO $$ 
BEGIN
  -- Drop policies on invoices (if table exists)
  ...
  -- Drop policies on invoice_items (if table exists)
...
-- Remove invoice_id from order_services (safe)
ALTER TABLE IF EXISTS public.order_services DROP COLUMN IF EXISTS invoice_id;
...
ALTER TABLE public.invoices ADD CONSTRAINT invoices_number_company_unique UNIQUE (company_id, invoice_number);
