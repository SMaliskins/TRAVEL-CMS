...
export default function DirectoryPage() {
  ...
  useEffect(() => {
    ...
    const scrollY = params.get("scroll");
    if (scrollY) {
      const y = parseInt(scrollY, 10);
      if (!isNaN(y) && y > 0) scrollRestoreRef.current = y;
      window.history.replaceState({}, "", window.location.pathname);
    } else {
      const stored = sessionStorage.getItem("directory.scrollY");
      if (stored) {
  ...
  useEffect(() => {
    if (scrollRestoreRef.current !== null && !loading) {
      ...
      const id = setTimeout(() => window.scrollTo({ top: y, behavior: "auto" }), 200);
  ...
  return (
    <div className="booking-modern-container">
      <div className="mx-auto max-w-[1400px] space-y-6">
            ...
            <div className="flex items-center gap-4">
              <button
                onClick={() => {
                  ...
                  if (y > 0) {
                    router.push(`/directory/new?from=directory&scroll=${y}`);
                  } else {
        ...
        {/* Contacts panel - search + table / loading / empty */}
