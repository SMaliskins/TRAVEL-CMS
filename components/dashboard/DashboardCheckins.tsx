...
const labels = {
  en: { title: "Flight Check-in", open: "OPEN", upcoming: "Opens in", closing: "Closes in", empty: "No upcoming check-ins", viewOrder: "Order" },
  ru: { title: "Онлайн-регистрация", open: "ОТКРЫТ", upcoming: "Откроется через", closing: "Закроется через", empty: "Нет ближайших регистраций", viewOrder: "Заказ" },
  lv: { title: "Lidojuma reģistrācija", open: "ATVĒRTS", upcoming: "Atvērsies pēc", closing: "Aizvērsies pēc", empty: "Nav tuvāko reģistrāciju", viewOrder: "Pasūtījums" },
};
...
export default function DashboardCheckins() {
  ...
  if (isLoading || checkins.length === 0) return null;
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-3">
      <div className="flex items-center gap-1.5 mb-2">
        ...
        {checkins.some((c) => c.status === "open" || c.status === "closing_soon") && (
          <span className="ml-auto flex h-5 min-w-[20px] items-center justify-center rounded-full bg-blue-500 px-1.5 text-[10px] font-bold text-white">
            {checkins.filter((c) => c.status === "open" || c.status === "closing_soon").length}
          </span>
        )}
      </div>
