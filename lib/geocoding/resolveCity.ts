/**
 * MAP-01 Step 2: city resolver with automatic growth.
 *
 * Resolution order (fast → slow, each layer fills the cache for next time):
 *   1. normalize(input) — lower, strip diacritics, trim
 *   2. LOCALE_ALIASES — hard-coded lv/ru → en for bulk existing spellings
 *   3. city_geocache table — previously resolved (any source)
 *   4. BUILTIN_CITIES — shipped coordinate table (~160 popular cities)
 *   5. Nominatim (OpenStreetMap) — external HTTPS call, throttled to 1 req/s
 *   6. COUNTRY_FALLBACK — approximate point on the country (marker still appears)
 *   7. unmapped — recorded so we don't keep calling Nominatim for the same miss
 *
 * Every success from layers 4/5/6 is INSERTed into city_geocache, so:
 *   - The second call for the same city is O(1) from DB.
 *   - Supervisors can manually correct bad entries via a future admin UI.
 *   - New cities (Tashkent, Bishkek, Ulaanbaatar, …) are added automatically.
 */
...
/** lower + strip diacritics + collapse whitespace */
...
// Compiled from real countries_cities values observed in the DB.
// Add more here when the "unmapped" log shows recurring misses.
// ────────────────────────────────────────────────────────────────────────────
const LOCALE_ALIASES: Record<string, string> = {
  ...
  // Asia
  puketa: "Phuket",
  pukteta: "Phuket", // known typo in existing data
  krabi_lv: "Krabi",
  samui_lv: "Koh Samui",
  "koh samui": "Koh Samui",
  bangkoka: "Bangkok",
  ...
  majami: "Miami",
  lasvegasa: "Las Vegas",
  "san francisco_lv": "San Francisco",
  toronto_lv: "Toronto",
  vankuvera: "Vancouver",
  meksika: "Mexico City",
  "mexico city_lv": "Mexico City",
  kankunas: "Cancun",
  kanka: "Cancun",
  cancun: "Cancun",
...
// ────────────────────────────────────────────────────────────────────────────
// Built-in coordinates (seed).  These are used if the cache table has no row
// yet, and are then persisted to the cache so the next request is a DB hit.
...
const BUILTIN_CITIES: Record<string, { lat: number; lng: number; canonical: string; country: string }> = {
  ...
  hvar: { lat: 43.1729, lng: 16.4411, canonical: "Hvar", country: "Croatia" },
  ...
  algarve: { lat: 37.0179, lng: -7.9304, canonical: "Algarve", country: "Portugal" },
  madeira: { lat: 32.6669, lng: -16.9241, canonical: "Madeira", country: "Portugal" },
  azores: { lat: 37.7412, lng: -25.6756, canonical: "Azores", country: "Portugal" },
  ...
  lefkada: { lat: 38.7066, lng: 20.653, canonical: "Lefkada", country: "Greece" },
  ...
  taormina: { lat: 37.8516, lng: 15.2853, canonical: "Taormina", country: "Italy" },
  ...
  seychelles: { lat: -4.6796, lng: 55.492, canonical: "Seychelles", country: "Seychelles" },
  ...
  "chiang mai": { lat: 18.7883, lng: 98.9853, canonical: "Chiang Mai", country: "Thailand" },
  ...
  langkawi: { lat: 6.35, lng: 99.8, canonical: "Langkawi", country: "Malaysia" },
...
// Country-level fallback — only if the city cannot be resolved at all.
// Marker appears at the country centroid with approximate=true.
const COUNTRY_FALLBACK: Record<string, { lat: number; lng: number; country: string }> = {
  ...
  "north macedonia": { lat: 41.6, lng: 21.7, country: "North Macedonia" },
  ...
  iceland: { lat: 64.9, lng: -19.0, country: "Iceland" },
  ...
  tajikistan: { lat: 38.9, lng: 71.3, country: "Tajikistan" },
  turkmenistan: { lat: 38.9, lng: 59.6, country: "Turkmenistan" },
  ...
  "saudi arabia": { lat: 23.9, lng: 45.1, country: "Saudi Arabia" },
...
async function callNominatim(city: string, country?: string): Promise<NominatimHit | null> {
  ...
  try {
    ...
    console.error("[resolveCity] Nominatim call failed:", err);
    return null;
  }
}
...
async function readCache(queryNorm: string, countryNorm: string): Promise<CacheRow | null> {
  const { data, error } = await supabaseAdmin
    .from("city_geocache")
    .select("city, country, lat, lng, source, approximate")
    .eq("query_norm", queryNorm)
    .eq("country_norm", countryNorm)
    .maybeSingle();
  if (error) {
    console.error("[resolveCity] readCache error:", error);
    return null;
  }
...
async function writeCache(
  ...
  if (error) console.error("[resolveCity] writeCache error:", error);
}

/**
 * Resolve a single (city, country) pair.  Always returns coordinates
 * (approximate country centroid if the city itself is unresolvable); only
 * returns null for completely unusable input (empty city, no country).
 */
export async function resolveCity(
  cityIn: string,
  countryIn?: string
): Promise<ResolvedCity | null> {
  const rawCity = (cityIn || "").trim();
  if (!rawCity) return null;
  const rawCountry = (countryIn || "").trim();

  const cityNorm = normalizeCityQuery(rawCity);
  const countryNorm = normalizeCityQuery(rawCountry);

  // 1) cache
  const cached = await readCache(cityNorm, countryNorm);
  if (cached && cached.lat !== null && cached.lng !== null) {
    return {
      lat: Number(cached.lat),
      lng: Number(cached.lng),
      city: cached.city || rawCity,
      country: cached.country || rawCountry,
      approximate: cached.approximate,
      source: cached.source as ResolvedCity["source"],
    };
  }
  if (cached && cached.source === "unmapped") {
    // Don't hammer Nominatim on a known miss; still return country fallback.
    const cf = COUNTRY_FALLBACK[countryNorm];
    if (cf) {
      return { lat: cf.lat, lng: cf.lng, city: rawCity, country: cf.country, approximate: true, source: "country-fallback" };
    }
    return null;
  }

  // 2) alias
  const alias = LOCALE_ALIASES[cityNorm];
  const candidates: string[] = [];
  if (alias) candidates.push(normalizeCityQuery(alias));
  candidates.push(cityNorm);
  ...
  for (const key of candidates) {
    ...
    await writeCache(cityNorm, countryNorm, {
      city: resolved.city,
  ...
  // 4) Nominatim
  const hit = await callNominatim(rawCity, rawCountry);
  if (hit && hit.lat && hit.lon) {
    const lat = Number(hit.lat);
    const lng = Number(hit.lon);
    if (Number.isFinite(lat) && Number.isFinite(lng)) {
      const resolvedCity = rawCity;
      const resolvedCountry = hit.address?.country || rawCountry || "";
      await writeCache(cityNorm, countryNorm, {
        city: resolvedCity,
  ...
  const cf = COUNTRY_FALLBACK[countryNorm];
  if (cf) {
    // Remember as unmapped so we don't retry Nominatim every request;
    // marker still gets shown via country-fallback lat/lng we compute each call.
    await writeCache(cityNorm, countryNorm, {
      city: rawCity,
      country: cf.country,
      lat: null,
...
/**
 * Batch-resolve a list of (city, country) pairs, de-duplicating identical
 * queries so the dashboard hits Nominatim at most once per unique city.
 */
export async function resolveCitiesBatch(
  pairs: { city: string; country?: string }[]
): Promise<Map<string, ResolvedCity | null>> {
  const keyOf = (c: string, country?: string) =>
    `${normalizeCityQuery(c)}|${normalizeCityQuery(country || "")}`;
  const out = new Map<string, ResolvedCity | null>();
  const seen = new Set<string>();

  for (const pair of pairs) {
    const key = keyOf(pair.city, pair.country);
    if (seen.has(key)) continue;
    seen.add(key);
    const resolved = await resolveCity(pair.city, pair.country);
