...
| SUPINV-PERIODIC | Finance | Auto-Periodic for periodic suppliers (Directory flag → default + backfill) | DB→CW→QA | IN_PROGRESS | feature/x | [27.04] Step 2.1 DB done. Step 2.2 done: `SupplierDetails.isPeriodicSupplier` in types, GET (list+detail) returns the flag, PUT/POST persist it, Directory form has a blue "Issues periodic invoices" checkbox in Supplier Details. No behaviour change yet on order services. Next: 2.3 default `supplier_invoice_requirement='periodic'` on service create when supplier is flagged; 2.4 backfill across active orders when flag turned on. |
