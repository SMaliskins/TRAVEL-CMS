...
| SUPINV-PERIODIC | Finance | Auto-Periodic for periodic suppliers (Directory flag → default + backfill) | DB→CW→QA | IN_PROGRESS | feature/x | [27.04] Step 2.1 DB done: column `party.is_periodic_supplier boolean NOT NULL DEFAULT false` added with partial index `idx_party_is_periodic_supplier`. Next: 2.2 Directory UI checkbox + API field; 2.3 default `supplier_invoice_requirement='periodic'` on service create when supplier is flagged; 2.4 backfill across active orders when flag turned on. |
